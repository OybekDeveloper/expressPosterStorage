document.addEventListener('DOMContentLoaded', () => {
  ...
  const quickRanges = {
    ...
    yesterday: { label: 'Вчера', from: new Date(today.setDate(today.getDate() - 1)), to: new Date(today.setDate(today.getDate() - 1)) },
    last7Days: { label: 'Последняя неделя', from: new Date(today.setDate(today.getDate() - 6)), to: today },
    last30Days: { label: 'Последний месяц', from: new Date(today.setDate(today.getDate() - 29)), to: today },
  };
  ...
  const fp = flatpickr(dateRangeInput, {
    ...
    onChange: (selectedDates) => {
      if (selectedDates.length === 2) {
        calendarToggle.textContent = `📅 ${formatDate(selectedDates[0])} - ${formatDate(selectedDates[1])}`;
      }
    },
  });
  ...
  async function fetchInitialData() {
    try {
      const [products, ingredients, stores] = await Promise.all([
        axios.get('/api/poster/fetch-poster-api', { params: { token, endpoint: 'menu.getProducts' } }),
        axios.get('/api/poster/fetch-poster-api', { params: { token, endpoint: 'menu.getIngredients' } }),
        axios.get('/api/poster/fetch-poster-api', { params: { token, endpoint: 'storage.getStorages' } }),
      ]);
  ...
  exportButton.addEventListener('click', async () => {
    ...
    try {
      // Fetch export data
      const response = await axios.get('/api/poster/fetch-export-data', {
      ...
      suppliesData = await Promise.all(
        suppliesData.map(async (item) => {
          try {
            const dataFetch = await axios.get('/api/poster/fetch-poster-api', {
